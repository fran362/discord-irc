    ...
    if (discordChannelName) {
      ...
      if(this.isCommandMessage(text)) {
          ...
          channelUser.forEach(function(user) {
             ...
             userList += signTag + user.username + ', '
          });

          userList = userList.substring(0, userList.length - 2)
          userList += ' : online ' + onlineCnt + ', offline ' + offlineCnt + ', idle ' + idleCnt + ' = Total ' + channelUser.length 

          this.sendToIRC(userList, true)
          return
      }
