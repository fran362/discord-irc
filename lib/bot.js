...
class Bot {
  ...
  attachListeners() {
    ...
    this.discord.on('message', message => {
      process.stdout.write('MSG >> ' + message + os.EOL);
  ...
  sendToIRC(message) {
	...
	switch(commands[0]) {
		...
		case '!deop' :
			...
			break;
		default :
			this.ircClient.say(ircChannel, 'Command is not support.');
			break;
	}
	...
	// this.ircClient.say(ircChannel, text);
        ...
        if (text !== '') {
          ...
          process.stdout.write('Sending message to IRC : ' + ircChannel + ' > ' +  text + os.EOL);
          this.ircClient.say(ircChannel, text);
        }
        ...
        if (message.attachments && message.attachments.length) {
          message.attachments.forEach(a => {
            ...
            process.stdout.write('Sending attachment URL to IRC : ' + ircChannel + ' > ' +  urlMessage + os.EOL);
  ...
  sendToDiscord(author, channel, text) {
    ...
    if (discordChannelName) {
      ...
      process.stdout.write('Sending message to Discord : ' + withAuthor + ' / CH:' +  channel + ' > ' + discordChannelName + os.EOL);
